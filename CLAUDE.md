...
| 8 | **Linux-style signal subsystem** - full sigaction table, `kill()` syscall, htop-style picker | 🟡 phases 1-3 shipped (PR #154): per-task handler table + scheduler-driven default-terminate delivery (`signal.{h,c}`, `7f9ba7a`); `SYS_KILL(37)` + `SYS_SIGNAL(48)` + userspace stubs (`c11dca2`); Ctrl+C → SIGINT migration, `g_sigint` / `keyboard_sigint_consume` removed, shell tasks install `SIG_IGN` (`449ded6`).  Phase 4 (ring-3 trampoline + `sigreturn` so user-installed handlers actually run) + htop-style picker are the remaining items. |
| 9 | **Preemption hardening** - interrupt-safe `schedule()`, per-task tick accounting, runtime-tunable quantum, busy-loop ktest | 🟡 phases 1 + 2 + 4 shipped (PR #154): `in_schedule` re-entrancy guard + `irq_save_disable`/`irq_restore` around `schedule()` (`c4f9299`); per-task `kticks` accounting bumped in `timer_callback` and rendered in `/proc/tasks`, plus `test_preempt` ktest that spawns a no-yield busy-loop victim and asserts the victim's `kticks` advanced (`d1af5bb`).  Remaining: phase 3 (runtime-tunable `SCHED_QUANTUM` via shell/`/proc` knob).  Concurrent-yield stress is implicitly covered by the existing 4-shell + bg-ktest concurrent execution. |
