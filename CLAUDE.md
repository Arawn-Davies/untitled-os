...
| 9 | **Preemption hardening** - interrupt-safe `schedule()`, per-task tick accounting, runtime-tunable quantum, busy-loop ktest | 🟡 phase 1 shipped (PR #154, `c4f9299`): `in_schedule` re-entrancy guard cleared before `task_switch` (fresh tasks bypass the epilogue) + `irq_save_disable` / `irq_restore` wrap the critical section so caller IF is preserved across every exit path.  Remaining phases: per-task tick accounting, runtime-tunable `SCHED_QUANTUM`, busy-loop + concurrent-yield ktests. |
